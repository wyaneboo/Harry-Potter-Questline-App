...
/**
 * QuestReward - Contains details about rewards earned from completing a quest.
 * Used to display in the completion animation popup.
 */
...
interface GameContextType {
  ...
  // Returns QuestReward if the quest was completed, otherwise undefined
  moveQuest: (
    questId: string,
  ...
  addQuest: (
    projectId: string,
    data: Omit<CreateQuestData, "project_id">,
  ) => Promise<string>;
  ...
  rewardQuestCompletion: (difficulty: QuestDifficulty) => Promise<QuestReward>;
...
export function GameProvider({ children }: GameProviderProps) {
   ...
   */
  const rewardQuestCompletion = useCallback(
    async (difficulty: QuestDifficulty): Promise<QuestReward> => {
      const reward: QuestReward = { xp: 0, galleons: 0 };
      ...
      try {
        ...
        if (Math.random() < dropChances[difficulty]) {
          const droppedItem = await addRandomDrop(); // Give random item
          if (droppedItem) {
            reward.drop = {
        ...
        console.log(
          `🎉 Quest completed! +${xpReward} XP, +${galleonReward} Galleons${reward.drop ? ` + ${reward.drop.name}` : ""}`,
        );
      } catch (error) {
  ...
  const moveQuest = useCallback(
    async (
      ...
      try {
        ...
        if (newStatus === "todo") {
          ...
          if (oldStatus !== "done") {
            reward = await rewardQuestCompletion(quest.difficulty);
          }
        }
    ...
    [loadProjects, rewardQuestCompletion],
  );
  ...
  const addQuest = useCallback(
    async (projectId: string, data: Omit<CreateQuestData, "project_id">) => {
      try {
  ...
  const updateQuest = useCallback(
    async (questId: string, data: UpdateQuestData) => {
      try {
